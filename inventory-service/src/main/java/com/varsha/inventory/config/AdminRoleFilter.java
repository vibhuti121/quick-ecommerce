 ...
 * requires the gateway-set {@code X-User-Role=ADMIN} header on every {@code /api/inventory/admin/**}
 * request, so a misroute that skips the gateway's role check still fails closed here.
 *
 * <p>Only the {@code /admin/} paths are guarded — the saga reservation hooks
 * ({@code /api/inventory/reservations/**}) are internal calls from order-service and carry no role
 * header, so they intentionally fall through untouched.
 *
...
public class AdminRoleFilter extends OncePerRequestFilter {
    ...
    private static final String ADMIN_PREFIX = "/api/inventory/admin";
