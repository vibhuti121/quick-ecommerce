...
- **2026-06-20** — **Fruit XI fan-box SHIPPED to `main`** (PR #45 `af8184b`, CI green → merged `bd2e67e`). Legal FIFA-WC-2026 × fruit engagement surface — pick a national team → build a starting XI of real catalogue fruits in the team's kit colours → share → add the deduped/honey-excluded fan-box to cart. **Streaming/restreaming permanently REFUSED** (copyright; link out only, never embed). Backend `catalog-service` under `/api/catalog/fruit-xi` (teams/box/autofill — all logic server-side, deterministic autofill so §8 sim-harness rule doesn't apply, 14 unit tests + live gateway smoke green). Frontend = repo's **first `react-router-dom`** (presentation only). Same round **codified two standing conventions**: **§9 backend-owns-logic** (no logic/validation/scoring/business-data in `frontend/` — split at `/varsha` scope time) and **§10 path-based page routing** (full-screen pages get real routes, not `?query`) into CLAUDE.md + varsha-routing + the FE/BE decision-maker docs. Follow-up **a11y fix** (PR #46 `4031f28`, CI green → merged `39a7dc8`): fixed AA contrast on kit-coloured jerseys/share-card/button (worst Morocco 1.29:1, white-on-white share cards) — Lighthouse a11y 94→100. **Merged to `main`, NOT yet deployed to mallde.in** (box redeploy is founder-gated). Pending: `~/.claude/commands/varsha.md` skill-contract edit still blocked by the auto-mode self-mod guard (rule enforced via varsha-routing meanwhile).
